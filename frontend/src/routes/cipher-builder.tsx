...
export const Route = createFileRoute("/cipher-builder")({
  component: CipherBuilderPage,
});
...
const STORAGE_KEY = "aqry-cipher-builder";
...
const DEFAULT_STATE: GuessBuilderState = {
  ...
  prompt: "",
  acceptedAnswers: ["", "", ""],
  successTitle: "Şifre çözüldü! 🔓",
  successDescription: "",
  retryEnabled: true,
  offerEnabled: false,
  offerTitle: "Çözümün perde arkasını gör",
  offerDescription: "Şifrenin nasıl kurulduğunu ve alternatif çözüm yolunu gör.",
  offerPrice: STANDARD_OFFER_PRICE,
};

type CipherStarterId =
  | "code"
  | "word"
  | "visual"
  | "pattern"
...
const CIPHER_STARTERS: CipherStarter[] = [
  {
    id: "code",
    title: "Bir kod buldursun",
    description:
      "Kullanıcı ipuçlarını birleştirip sayı veya kısa bir kod bulsun.",
    titlePlaceholder:
      "Örn. 4 haneli şifreyi çöz",
    descriptionPlaceholder:
      "Örn. Üç ipucunu birleştir ve doğru kodu bul.",
    promptPlaceholder:
      "Örn. 2 → 6, 3 → 12, 4 → 20 ise 5 → ?",
    answerHint:
      "Sonraki adımda doğru kodu yazacaksın. Örn. 30",
  },
  {
    id: "word",
    title: "Bir kelime buldursun",
    description:
      "Harfler, çağrışımlar veya kısa ipuçlarından tek bir kelime bulunsun.",
    titlePlaceholder:
      "Örn. Gizli kelimeyi bul",
    descriptionPlaceholder:
      "Örn. İpuçlarını birleştir ve aradığımız kelimeyi yaz.",
    promptPlaceholder:
      "Örn. Gece + gökyüzü + ışık → ?",
    answerHint:
      "Sonraki adımda doğru kelimeyi ve kabul edeceğin alternatif yazımları ekleyeceksin.",
  },
  {
    id: "visual",
    title: "Görselde gizli şeyi buldursun",
    description:
      "Bir görselde saklanan sayı, kelime, nesne veya işaret bulunsun.",
    titlePlaceholder:
      "Örn. Görselde saklanan sayıyı bul",
    descriptionPlaceholder:
      "Örn. Görsele dikkatlice bak. Küçük bir detay sana cevabı verecek.",
    promptPlaceholder:
      "Örn. Görselde gizlenen 3 haneli sayı nedir?",
    answerHint:
      "Önce yukarıdan görseli yükle; sonraki adımda görselde sakladığın doğru cevabı yaz.",
  },
  {
    id: "pattern",
    title: "Bir örüntüyü tamamlasın",
    description:
      "Sayı, harf veya sembol dizisinin kuralını keşfedip sıradakini bulsun.",
    titlePlaceholder:
      "Örn. Sıradaki sayı ne?",
    descriptionPlaceholder:
      "Örn. Dizinin kuralını keşfet ve eksik parçayı tamamla.",
    promptPlaceholder:
      "Örn. 1, 4, 9, 16, ?",
    answerHint:
      "Sonraki adımda dizinin doğru devamını yazacaksın. Örn. 25",
  },
  {
    id: "hidden",
    title: "Gizli mesajı çözsün",
    description:
      "Metnin, emojilerin veya sembollerin içine saklanan mesaj keşfedilsin.",
    titlePlaceholder:
      "Örn. Gizli mesajı çöz",
    descriptionPlaceholder:
      "Örn. Her şey göründüğü gibi değil. Mesaj satırların arasında saklı.",
    promptPlaceholder:
      "Örn. Her satırın ilk harfini birleştir. Hangi kelime çıkıyor?",
    answerHint:
      "Sonraki adımda ortaya çıkması gereken gizli kelimeyi veya mesajı yaz.",
  },
  {
    id: "custom",
    title: "Kendim oluşturacağım",
    description:
      "Kendi şifreni veya bulmacanı sıfırdan kur.",
    titlePlaceholder:
      "İçeriğine kısa ve merak uyandıran bir başlık yaz",
    descriptionPlaceholder:
      "Kullanıcıya ne yapması gerektiğini bir cümlede anlat.",
    promptPlaceholder:
      "Çözmesini istediğin şifreyi veya ana ipucunu buraya yaz.",
    answerHint:
      "Sonraki adımda AQRYO'nun doğru kabul edeceği cevabı tanımlayacaksın.",
  },
];

function CipherBuilderPage() {
...
function ContentStep({
  ...
  const [starterId, setStarterId] =
    useState<CipherStarterId>("code");

  const starter =
    CIPHER_STARTERS.find(
      (item) => item.id === starterId,
    ) ?? CIPHER_STARTERS[0];

  return (
    <BuilderCard
      eyebrow="1 · Şifreyi kur"
      title="Kullanıcı neyi çözsün?"
      description="Önce basitçe yapmak istediğin şeyi seç. Teknik bir şifre türü bilmen gerekmiyor."
    >
      <div className="grid gap-3 sm:grid-cols-2">
        {CIPHER_STARTERS.map((item) => {
          const selected =
            starterId === item.id;
      ...
      <div className="mt-5 rounded-[18px] border border-primary/15 bg-primary/[0.025] p-4">
        <p className="text-[13px] font-black">
          Şimdi sadece 3 şey yap
        </p>
        ...
        <div className="mt-3 grid gap-2 text-[11px] leading-5 text-muted-foreground">
          <p>
            ...
            Başlığı ve kısa açıklamayı yaz.
          </p>
          <p>
            <strong className="text-foreground">
              2.
            </strong>{" "}
            Şifreni veya ana ipucunu yaz. Gerekirse görsel yükle.
          </p>
          <p>
            <strong className="text-foreground">
              3.
      ...
      <div className="mt-5">
        <ImageUploader
          ...
          label="Görsel kullanmak ister misin?"
          helperText="Zorunlu değil. Görselin şifrenin veya ipucunun bir parçasıysa JPG ya da PNG yükle."
        />
      </div>
...
function AnswerStep({
  ...
  return (
    <BuilderCard
      eyebrow="2 · Doğru şifre"
      title="Doğru cevap ne?"
      description="Kullanıcının bulması gereken cevabı yaz. Gerekirse aynı cevabın farklı yazımlarını da ekleyebilirsin."
    >
        ...
        {answers.map((answer, index) => (
          <div key={index} className="flex gap-2">
            <input
              ...
              placeholder={index === 0 ? "Ana doğru cevap" : `Alternatif yazım ${index}`}
            />
            {index > 0 ? (
              <button
                type="button"
  ...
  return (
    <div>
      ...
      <div className="overflow-hidden rounded-[24px] border border-border bg-white shadow-[0_16px_45px_rgba(22,12,34,0.05)]">
        ...
        <div className="p-5">
          <p className="text-[12px] font-black">{state.prompt || "Şifreyi çöz"}</p>
...
function stepLabel(step: BuilderStep) {
  ...
  if (step === "answer") return "Doğru şifre";
